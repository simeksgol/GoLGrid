...
static __not_inline void ObjCellList_make_illegal (ObjCellList *obj)
{
	if (!obj)
		return (void) ffsc (__func__);
...
{
	if (!src_obj || !src_obj->cell || !dst_obj || !dst_obj->cell)
		return ffsc (__func__);
...
{
	if (!in_obj || !in_obj->cell || in_obj->obj_rect.width > 254 ||in_obj->obj_rect.height > 254 || !out_obj || !out_obj->cell)
		return ffsc (__func__);
