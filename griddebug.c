static __not_inline void GoLGrid_debug_compare (const GoLGrid *obj_gg, const GoLGrid *ref_gg)
{
...
static __not_inline s32 GoLGrid_debug_search_pop_x_on (const GoLGrid *gg)
{
	if (!gg || !gg->grid)
...
static __not_inline s32 GoLGrid_debug_search_pop_x_off (const GoLGrid *gg)
{
	if (!gg || !gg->grid)
		return ffsc (__func__);
...
static __not_inline s32 GoLGrid_debug_search_pop_y_on (const GoLGrid *gg)
{
	if (!gg || !gg->grid)
		return ffsc (__func__);
...
static __not_inline s32 GoLGrid_debug_search_pop_y_off (const GoLGrid *gg)
{
	if (!gg || !gg->grid)
		return ffsc (__func__);
...
static __not_inline void GoLGrid_debug_verify_consistency (const GoLGrid *gg)
{
	if (!gg || !gg->grid)
		return (void) ffsc (__func__);
...
static __not_inline void GoLGrid_debug_clear_entire_grid (GoLGrid *gg)
{
	if (!gg || !gg->grid)
		return (void) ffsc (__func__);
...
static __not_inline void GoLGrid_debug_evolve_column (const u64 *in_entry, u64 *out_entry, s32 row_count)
{
...
static __not_inline void GoLGrid_debug_evolve_between_columns (const u64 *in_entry_left, const u64 *in_entry_right, u64 *out_entry_left, u64 *out_entry_right, s32 row_count)
{
...
static __not_inline void GoLGrid_debug_set_population_limits_from_scratch (GoLGrid *gg)
{
	s32 pop_x_on = GoLGrid_debug_search_pop_x_on (gg);
	s32 pop_x_off = GoLGrid_debug_search_pop_x_off (gg);
	s32 pop_y_on = GoLGrid_debug_search_pop_y_on (gg);
...
static __not_inline void GoLGrid_debug_reference_evolve (const GoLGrid *in_gg, GoLGrid *out_gg)
{
...
static __not_inline void GoLGrid_debug_or_slow_expansion_test_pattern (GoLGrid *gg, int left_x, int top_y)
{
	if (!gg || !gg->grid)
		return (void) ffsc (__func__);
